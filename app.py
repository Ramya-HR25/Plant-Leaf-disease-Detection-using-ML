    ...
    try:
        ...
        for file in files:
            ...
            if (top1_conf < MIN_CONFIDENCE) or ((top1_conf - top2_conf) < MIN_MARGIN):
                ...
                treatment_info = {
                    'description': 'The system requires high confidence (75% or above) to ensure accurate predictions. This image did not meet the confidence threshold, which helps prevent incorrect diagnoses. This may occur if the image is unclear, the leaf is not from the supported plant species, or lighting conditions are poor.',
                    'treatment': 'Please try with a clearer, well-lit image of a plant leaf from the supported categories. Ensure the leaf fills most of the frame and shows clear disease symptoms if present.',
                    'supplement': 'N/A',
                    'link': '#'
                }
                results.append({
