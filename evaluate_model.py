...
# Configuration
DATASET_DIR = 'PlantVillage'  # change if you use a different folder (e.g., PlantVillage_reliable)
IMG_SIZE = (224, 224)
BATCH_SIZE = 32

# Optional: support an augmented dataset layout
...
# Helper to support very long Windows paths (if required)
...
num_samples = getattr(val_generator, 'samples', 0)
if num_samples == 0:
    raise RuntimeError(
        f"Validation generator contains no samples. Check DATASET_DIR ('{DATASET_DIR}') or VALID_DIR paths."
    )


# Run prediction
# model.predict will iterate over the generator; specify steps to avoid ambiguity
steps = math.ceil(num_samples / float(BATCH_SIZE))
